...
// BoardGUI Class - Main container for animations
...
// Group Class - Container for multiple graphic objects
...
PixiJSEdu.Polygon = class Polygon extends PIXI.Container {
  ...
  _updatePosition() {
    // Sollte beide Pivots berücksichtigen
    super.x = this._visualX + this._rotationPivotX - this._pivotX;
    super.y = this._visualY + this._rotationPivotY - this._pivotY;
  }
  _updateRotationPivot() {
    this.pivot.set(this._rotationPivotX, this._rotationPivotY);
    // Position muss auch aktualisiert werden, wenn sich der Pivot ändert
    this._updatePosition();
  }
  set x(value) {
    this._visualX = value;
  ...
  // Destroy-Methode hinzufügen/erweitern
  destroy() {
...
PixiJSEdu.Arrow = class Arrow extends PIXI.Container {
  ...
  constructor(
    ...
    this._rotationDegrees = 0; // Speichere Rotation in Grad
  ...
  // Rotation setter und getter (in Grad)
  set rotation(degrees) {
    this._rotationDegrees = degrees;
    // Konvertiere Grad zu Bogenmaß für PIXI
    super.rotation = degrees * (Math.PI / 180);
  }
  ...
  // Getter für die Start- und Endkoordinaten
  get startX() {
    return this._startX;
  }
...
PixiJSEdu.Parallelogram = class Parallelogram extends PIXI.Container {
  ...
  constructor(
    ...
    this._fillColor = fillColor;
    this._alpha = 1.0;
    this._savedAlpha = 1.0; // Speichere den Alpha-Wert separat für visible-Toggle
    this._visible = true;
    ...
    // Container für verschiedene Elemente
    ...
    // Erstelle die Vektorpfeile (initial versteckt)
  ...
  _draw() {
    ...
    // Berechne die vier Eckpunkte des Parallelogramms
    ...
    // Der vierte Punkt ist die Summe der beiden Vektoren
    const resultant = this.getResultantEndpoint();
    const p4 = { x: resultant.x, y: resultant.y };

    // Zeichne das gefüllte Parallelogramm
    ...
    // Aktualisiere die Vektorpfeile wenn sie angezeigt werden sollen
  ...
  _updateVectorArrows() {
    ...
    // Erstelle neue Pfeile wenn gewünscht
  ...
  getResultantEndpoint() {
    // Berechne die Vektoren relativ zum Ursprung
    const v1 = {
  ...
  setAlpha(alpha) {
    this._alpha = Math.max(0, Math.min(1, alpha));
    this._savedAlpha = this._alpha; // Aktualisiere auch den gespeicherten Wert
    this._draw();
    return this;
  }
...
PixiJSEdu.PointLabel = class PointLabel extends PIXI.Container {
  ...
  _parseHTMLText(htmlText) {
    // Parst den HTML-Text und gibt ein Array von Text-Segmenten zurück
    const segments = [];
    ...
    while ((match = regex.exec(htmlText)) !== null) {
      ...
      segments.push({
        text: match[2],
        type: match[1], // 'sub' oder 'sup'
      });

      lastIndex = regex.lastIndex;
    }

    // Restlicher Text nach dem letzten Tag
  ...
  _createTextElements() {
    // Lösche alte Text-Elemente
    this.textContainer.removeChildren();

    const segments = this._parseHTMLText(this._text);
    let currentX = 0;

    // Erstelle Text-Objekte für jedes Segment
    segments.forEach((segment) => {
      ...
      // Vertikale Position für sub/sup
      if (segment.type === "sub") {
  ...
  _updatePosition() {
    // Position basierend auf Punkt und Offset berechnen
  ...
  setTextColor(color) {
    ...
    // Alle Text-Objekte aktualisieren
...
PixiJSEdu.LineLabel = class LineLabel extends PIXI.Container {
  ...
  _calculateLineAngle() {
    ...
    // Winkel in Bereich [0, 2π] normalisieren
  ...
  _isMoreHorizontal() {
    // Prüfe die Richtung der Linie basierend auf dx
    const dx = this._endX - this._startX;

    // Wenn dx positiv ist (Linie geht nach rechts), true
    // Wenn dx negativ ist (Linie geht nach links), false
    return dx >= 0;
  }
  ...
  _updatePosition() {
    ...
    // Normalenvektor (orthogonal zur Strecke)
    let normalX = -unitY;
    let normalY = unitX;

    // Entscheidung nur basierend auf dx
    if (dx < 0) {
      // Linie zeigt nach links: Normalenvektor umkehren
      normalX = -normalX;
      normalY = -normalY;
    }

    // Wenn geflippt, dann Normalenvektor umkehren
    if (this._flipped) {
      normalX = -normalX;
      normalY = -normalY;
    }
...
PixiJSEdu.CoordinateSystem = class CoordinateSystem extends PIXI.Container {
  static serializationMap = {
    ...
    methods: {
      ...
      addTicksAndLabels: {
        example: 'addTicksAndLabels(["0", "5", "10"], "right", 50, 0, 10)',
        info: {
          en: "Adds tick marks and labels with specified values",
          de: "Fügt Teilstriche und Beschriftungen hinzu mit angegebenen Werten",
        },
      },
    },
  };
  ...
  setColor(color) {
    ...
    if (this._rulers.top) this._rulers.top.setColor(color);
    if (this._rulers.bottom) this._rulers.bottom.setColor(color);
    if (this._rulers.left) this._rulers.left.setColor(color);
    if (this._rulers.right) this._rulers.right.setColor(color);
    return this;
  }
  setThickness(thickness) {
    this._thickness = thickness;
    ...
    if (this._rulers.top) this._rulers.top.setThickness(thickness);
    if (this._rulers.bottom) this._rulers.bottom.setThickness(thickness);
    if (this._rulers.left) this._rulers.left.setThickness(thickness);
    if (this._rulers.right) this._rulers.right.setThickness(thickness);
    return this;
  }
  addTicksAndLabels(
    values,
    ...
    if (this._rulers[direction]) {
      this.removeChild(this._rulers[direction]);
    }
    this._rulers[direction] = new PixiJSEdu.Ruler(
      direction,
      values,
      spacing,
      spacingOffset,
    ...
    this.addChild(this._rulers[direction]);
    return this;
  }
};
...
PixiJSEdu.SimpleSVG = class SimpleSVG extends PIXI.Container {
  ...
  // OPTIMIERUNG: Statischer Texture-Cache für SVGs
  ...
  constructor(svgString) {
    ...
    this._dragData = null;
    this._resolution = 2; // WICHTIG: Erhöht auf 2 für bessere Standard-Auflösung
    this._isLoading = false;
    this._loadingPromise = null;
    this._pendingOperations = [];

    // OPTIMIERUNG: Verzögertes Laden für bessere Performance
    this._needsRedraw = true;
    this._redrawTimeout = null;

    // OPTIMIERUNG: Cache-Key für dieses SVG
    this._cacheKey = null;

    this.borderGraphics = new PIXI.Graphics();
    this.addChild(this.borderGraphics);
    this.gruppe = new PIXI.Container(); // OPTIMIERUNG: Container statt Graphics
    ...
    // OPTIMIERUNG: Verzögertes initiales Laden
    this._scheduleRedraw();
  }

  // OPTIMIERUNG: Debounced Redraw für bessere Performance
  _scheduleRedraw() {
    ...
    }, 16); // Ein Frame Verzögerung
  }

  // OPTIMIERUNG: Cache-Key generieren
  _generateCacheKey() {
    const key = [
      this._originalSvgString.substring(0, 100), // Erste 100 Zeichen als Identifikation
      this._scale.x,
      this._scale.y,
      this._resolution,
  ...
  // OPTIMIERUNG: Cache bereinigen wenn zu groß
  _cleanupCache() {
    if (SimpleSVG.textureCache.size > SimpleSVG.MAX_CACHE_SIZE) {
      // Entferne die ältesten Einträge (erste Hälfte)
  ...
  _setupDragInteractivity() {
    ...
    // OPTIMIERUNG: Bound functions nur einmal erstellen
  ...
  destroy(options) {
    ...
    if (this.sprite) {
      this.sprite.destroy(false); // Texture nicht zerstören (könnte gecached sein)
    }
    ...
    // OPTIMIERUNG: Aus Board entfernen
      ...
      try {
        ...
        img.onload = () => {
          const textureOptions = {
            resolution: this._resolution, // Nur die eingestellte Resolution verwenden
            antialias: true, // Aktiviere Antialiasing
          };
  ...
  // OPTIMIERUNG: Separiere Texture-Anwendung
  _applyTexture(texture) {
    if (this.sprite) {
      this.gruppe.removeChild(this.sprite);
      this.sprite.destroy(false); // Texture nicht zerstören
      this.sprite = null;
    }
    ...
    // Skalierung basierend nur auf der Resolution
  ...
  _updateBorder() {
    ...
    // OPTIMIERUNG: Border nur bei Debug oder wenn explizit gewünscht
    if (this._showBorder) {
  ...
  setScale(scaleX, scaleY) {
    // OPTIMIERUNG: Prüfe ob sich wirklich was ändert
  ...
  // OPTIMIERUNG: Direkte Transform-Methoden für Animationen
  setTransform(x, y, scaleX, scaleY, rotation) {
    // Setze Position ohne Neuzeichnen
    this._visualX = x;
    this._visualY = y;
    this._updatePosition();

    // Setze Skalierung direkt am Container
    if (this.gruppe) {
      this.gruppe.scale.set(scaleX, scaleY);
      this.gruppe.rotation = rotation * (Math.PI / 180);
    }

    // Speichere für späteren Gebrauch, aber KEIN Redraw
    this._scale.x = scaleX;
    this._scale.y = scaleY;
...
// OPTIMIERUNG: Statische Methode zum Cache leeren
